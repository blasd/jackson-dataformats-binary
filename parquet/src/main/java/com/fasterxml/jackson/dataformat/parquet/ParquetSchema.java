...
import com.fasterxml.jackson.dataformat.avro.deser.ParquetReaderFactory;

/**
 * Wrapper for Schema information needed to encode and decode Parquet-format
 * data.
 */
public class ParquetSchema extends AvroSchema
{
    /**
     * Format type id used by Jackson components to refer to Parquet Format.
     */
    public final static String TYPE_ID = "parquet";
    ...
    public ParquetSchema(Schema asch)
    {
       super(asch);
    }
     ...
     * @since 2.9
     */
    public AvroSchema withReaderSchema(ParquetSchema readerSchema)
...
r.getType().getName(), w.getFullName(), r.getFullName()));
    ...
    protected AvroStructureReader _constructReader() {
        return ParquetReaderFactory.createFor(_writerSchema);
    }
    ...
    public boolean equals(Object o) {
        ...
        ParquetSchema other = (ParquetSchema) o;
     ...
     * (by parquet codec) on top of binary codec, exposing content using (reader) schema
     * this instance was configured with.
     */
    private final static class Resolving extends ParquetSchema
    {
        ...
        @Override
        protected AvroStructureReader _constructReader() {
            return ParquetReaderFactory.createFor(_writerSchema, _readerSchema);
        }
        ...
        @Override
        public String toString() {
            return String.format("{ParquetSchema.Resolving: name=%s}", _writerSchema.getFullName());
        }
