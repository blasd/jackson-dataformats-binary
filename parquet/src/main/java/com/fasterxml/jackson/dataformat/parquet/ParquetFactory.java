 ...
 *
 * @see com.fasterxml.jackson.dataformat.parquet.apacheimpl.ApacheParquetFactory
 */
public class ParquetFactory extends JsonFactory
{
    private static final long serialVersionUID = 1L;

    public final static String FORMAT_NAME_PARQUET = "parquet";
     ...
     * factory instance.
     */
    public ParquetFactory() { this(null); }

    public ParquetFactory(ObjectCodec oc)
    {
        super(oc);
    ...
    protected ParquetFactory(ParquetFactory src, ObjectCodec oc)
    {
        super(src, oc);
    ...
    {
        _checkInvalidCopy(ParquetFactory.class);
        return new ParquetFactory(this, null);
    }
    ...
    protected Object readResolve() {
        return new ParquetFactory(this, _objectCodec);
    }
     ...
     */
    public final ParquetFactory configure(AvroParser.Feature f, boolean state)
    {
        if (state) {
            enable(f);
        } else {
     ...
     * (check {@link AvroParser.Feature} for list of features)
     */
    public ParquetFactory enable(AvroParser.Feature f) {
     ...
     * (check {@link AvroParser.Feature} for list of features)
     */
    public ParquetFactory disable(AvroParser.Feature f) {
        _avroParserFeatures &= ~f.getMask();
        return this;
    }
     ...
     */
    public final ParquetFactory configure(AvroGenerator.Feature f, boolean state) {
        if (state) {
            enable(f);
        } else {
            disable(f);
     ...
     * (check {@link AvroGenerator.Feature} for list of features)
     */
    public ParquetFactory enable(AvroGenerator.Feature f) {
     ...
     * (check {@link AvroGenerator.Feature} for list of features)
     */
    public ParquetFactory disable(AvroGenerator.Feature f) {
        _avroGeneratorFeatures &= ~f.getMask();
        return this;
    }
